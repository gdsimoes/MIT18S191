...
student = (name = "Guilherme Dias Simões", kerberos_id = "imNotAnMITStudent")
...
ex_1_1 = md"""
The case where $a = \sqrt{x}$ is trivial. Let's consider only the case where $\sqrt{x} < a$ because the remaining case is similar. If $\sqrt{x} < a$ then $\frac{\sqrt{x}}{a} < 1$ and $\frac{x}{a} < \sqrt{x}$. So $\frac{x}{a} < \sqrt{x} < a$.
""" 
...
function newton_sqrt(x, error_margin=0.01, a=x / 2) # a=x/2 is the default value of `a`
	new_a = x/a
	if abs(a - new_a) < error_margin
		return a
	else
		return newton_sqrt(x, error_margin, (a + new_a)/2)
	end
end
